...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from amyROM-devices Github (http://github.com/amyROM-devices)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:amyROM-devices+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def is_in_manifest(projectpath):
    ...
    # And the most important: amySnippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/amy.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories, fallback_branch = "rosie"):
    try:
        lm = ElementTree.parse(".repo/local_manifests/roomservice.xml")
        lm = lm.getroot()
    except:
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('amyROM-devices/%s already fetched to %s' % (repo_name, repo_target))
            continue
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    dependencies_path = repo_path + '/amy.dependencies'
    syncable_repos = []
    verify_repos = []
...
print("Repository for %s not found in the amyROM-devices Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
